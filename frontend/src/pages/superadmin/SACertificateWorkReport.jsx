...
import {
    ...
    XCircle, Clock, ExternalLink, Download, FileText, Users,
    BarChart3, GraduationCap, Filter, RefreshCw, Link2, Star,
    AlertCircle, Package, TrendingUp, Layers
} from 'lucide-react';
...
// ══════════════════════════════════════════════════════════════════════════════
const CertificateReport = ({ selectedBatch }) => {
    const [data, setData] = useState(null);
    const [loading, setLoading] = useState(false);
    const [expanded, setExpanded] = useState({});
    const [filter, setFilter] = useState('all'); // all | eligible_completion | obtained_completion | eligible_internship | obtained_internship
    ...
    const getCompletionStatus = s => s.completion_cert_at ? 'obtained' : s.completion_eligible ? 'eligible' : 'not_eligible';
    const getInternshipStatus = s => s.internship_cert_at ? 'obtained' : s.internship_eligible ? 'eligible' : 'not_eligible';
    ...
    const filters = [
        ...
        { key: 'obtained_completion',  label: 'Obtained — Completion' },
        { key: 'eligible_internship',  label: 'Eligible — Internship' },
        { key: 'obtained_internship',  label: 'Obtained — Internship' },
    ];
    ...
    return (
        <div>
            ...
            {batches.map(batch => {
                ...
                return (
                    <div key={batch.batch_id} className="glass-card" style={{ padding: 0, overflow: 'hidden', marginBottom: '1rem' }}>
                        {/* Batch header */}
                        <div onClick={() => setExpanded(e => ({ ...e, [batch.batch_id]: !open }))}
                            style={{
                                padding: '0.9rem 1.25rem', cursor: 'pointer', display: 'flex',
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                                <Badge color="#51cf66" bg="rgba(81,207,102,0.12)">{students.filter(s => !!s.completion_cert_at).length} completion certs</Badge>
                                <Badge color="#fcc419" bg="rgba(252,196,25,0.12)">{students.filter(s => !!s.internship_cert_at).length} internship certs</Badge>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                                    <thead>
                                        <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                                            {['Student','Roll #','Attendance','Projects','Tests','Feedback','Portfolio','Completion Cert','Internship Cert'].map(h => (
                                                <th key={h} style={{ padding: '0.75rem 1rem', textAlign: 'left', fontSize: '0.72rem', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700, whiteSpace: 'nowrap' }}>{h}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {students.map(s => {
                                            const cs = getCompletionStatus(s);
                                            const is = getInternshipStatus(s);
                                            return (
                                                <tr key={s.student_id} style={{ borderBottom: '1px solid var(--border-color)', transition: '0.15s' }}>
                                                    <td style={{ padding: '0.75rem 1rem' }}>
                                                        <div style={{ fontWeight: 600, color: 'var(--text-main)' }}>{s.student_name}</div>
                                                        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{s.email}</div>
                                                    </td>
                                                    <td style={{ padding: '0.75rem 1rem', color: 'var(--text-muted)', fontSize: 12 }}>{s.roll_number || '—'}</td>

                                                    {/* Attendance */}
                                                    <td style={{ padding: '0.75rem 1rem', minWidth: 100 }}>
                                                        <PctBar pct={s.att_pct} color={s.att_pct >= 80 ? '#51cf66' : s.att_pct >= 75 ? '#fcc419' : '#fa5252'} />
                                                        <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 3 }}>{s.att_present}/{s.att_total} days</div>
                                                    </td>

                                                    {/* Projects */}
                                                    <td style={{ padding: '0.75rem 1rem' }}>
                                                        <div style={{ fontWeight: 700, color: s.proj_avg != null && s.proj_avg >= 75 ? '#51cf66' : '#fa5252', fontSize: 14 }}>
                                                            {s.proj_avg != null ? `${s.proj_avg}%` : '—'}
                                                        </div>
                                                        <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>{s.proj_submitted}/{s.proj_released} submitted</div>
                                                    </td>

                                                    {/* Tests */}
                                                    <td style={{ padding: '0.75rem 1rem' }}>
                                                        <div style={{ fontWeight: 600, color: s.tests_released === 0 || s.test_pct === 100 ? '#51cf66' : '#fa5252', fontSize: 13 }}>
                                                    ...
                                                    {/* Feedback */}
                                                    <td style={{ padding: '0.75rem 1rem' }}>
                                                        <div style={{ fontWeight: 600, color: s.fb_released === 0 || s.fb_pct === 100 ? '#51cf66' : '#fa5252', fontSize: 13 }}>
            ...
            {batches.every(b => !b.students.filter(filterFn).length) && (
...
// ══════════════════════════════════════════════════════════════════════════════
const StudentWorkReport = ({ selectedBatch }) => {
    const [data, setData] = useState(null);
    const [loading, setLoading] = useState(false);
    const [expandedBatch, setExpandedBatch] = useState({});
    const [expandedStudent, setExpandedStudent] = useState({});
    const [filter, setFilter] = useState('all'); // all | submitted | graded | with_link | not_submitted
    ...
    const statusBadge = (status, github_link) => {
        ...
        return <Badge color="#868e96" bg="rgba(255,255,255,0.05)"><AlertCircle size={10} />Not Submitted</Badge>;
    };
                    ...
                    <div key={batch.batch_id} className="glass-card" style={{ padding: 0, overflow: 'hidden', marginBottom: '1rem' }}>
                        ...
                        <div onClick={() => setExpandedBatch(e => ({ ...e, [batch.batch_id]: !bOpen }))}
                            style={{
                                ...
                                justifyContent: 'space-between', alignItems: 'center',
                                background: 'rgba(255,255,255,0.02)',
                                borderBottom: bOpen ? '1px solid var(--border-color)' : 'none',
                            }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                {bOpen ? <ChevronDown size={16} color="var(--primary)" /> : <ChevronRight size={16} color="var(--primary)" />}
                                <div>
                                    <span style={{ fontWeight: 700, color: 'var(--text-main)' }}>{batch.batch_name}</span>
                                    <span style={{ marginLeft: 10, fontSize: 12, color: 'var(--text-muted)' }}>{batch.course_name}</span>
                                </div>
                        ...
                        {bOpen && (
                            <div style={{ padding: '0.5rem 0' }}>
                                {students.map(student => {
                                    ...
                                    const projPct = student.total_assigned > 0
                                        ? Math.round(student.submitted_count / student.total_assigned * 100) : 0;

                                    return (
                                        <div key={student.student_id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                                            {/* Student row */}
                                            <div onClick={() => setExpandedStudent(e => ({ ...e, [sKey]: !sOpen }))}
                                                style={{
                                                    padding: '0.75rem 1.25rem', cursor: 'pointer', display: 'flex',
                                                    alignItems: 'center', gap: 12,
                                                    background: sOpen ? 'rgba(255,255,255,0.02)' : 'transparent',
                                                }}>
                                                ...
                                                <div style={{ width: 200 }}>
                                                    <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 3, display: 'flex', justifyContent: 'space-between' }}>
                                                        <span>Projects</span>
                                                        <span>{student.submitted_count}/{student.total_assigned}</span>
                                                    </div>
                                                ...
                                                <div style={{ display: 'flex', gap: 6 }}>
                                                    ...
                                                    {student.projects.some(p => p.github_link) && (
                                                        <Badge color="#ae3ec9" bg="rgba(174,62,201,0.12)"><Link2 size={10} />Links</Badge>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            {sOpen && (
                                                <div style={{ padding: '0 1.25rem 0.75rem 3rem' }}>
                                                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                                                        <thead>
                                                            <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                                                                {['Project','Module','Type','Status','Marks','GitHub / File','Feedback','Submitted At'].map(h => (
                                                                    <th key={h} style={{ padding: '6px 10px', textAlign: 'left', fontSize: '0.7rem', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700, whiteSpace: 'nowrap' }}>{h}</th>
                                                                ))}
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            ...
                                                            {student.projects.map((p, i) => (
                                                                <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                                                    ...
                                                                    <td style={{ padding: '8px 10px' }}>
                                                                        <Badge
                                                                            color={p.release_type === 'capstone_project' ? '#f59f00' : '#748ffc'}
                                                                    ...
                                                                    <td style={{ padding: '8px 10px' }}>{statusBadge(p.status, p.github_link)}</td>
...
// MAIN EXPORT
// ══════════════════════════════════════════════════════════════════════════════
export const SACertificateWorkReport = ({ apiOverride }) => {
    const [tab, setTab] = useState('certificates');
    const [batches, setBatches] = useState([]);
    const [selectedBatch, setSelectedBatch] = useState('');

    useEffect(() => {
        reportsAPI.getBatches().then(r => setBatches(r.data.batches || [])).catch(() => {});
    }, []);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <select value={selectedBatch} onChange={e => setSelectedBatch(e.target.value)}
                        style={{ padding: '8px 12px', borderRadius: 8, background: 'var(--bg-dark)', border: '1px solid var(--border-color)', color: 'var(--text-main)', fontSize: 13, outline: 'none', cursor: 'pointer' }}>
            ...
            {/* Content */}
            {tab === 'certificates'  && <CertificateReport  selectedBatch={selectedBatch} />}
            {tab === 'student-work'  && <StudentWorkReport  selectedBatch={selectedBatch} />}
        </div>
    );
};
